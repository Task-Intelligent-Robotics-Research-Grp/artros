...
def launch_setup(context):
    moveit_configs = MoveItConfigsBuilder(robot_name='dual_robotiq_base_scene',
                                          package_name='dual_robotiq_moveit_config') \
                    .robot_description_semantic(Path('config')
                                                / 'dual_robotiq_base_scene.srdf',
                                                {'name': 'dual_robotiq_base_scene'}) \
                    .to_moveit_configs()
    wait_robot_description = Node(package='aist_bringup',
                                  executable='wait_for_robot_description',
                                  output='screen')
    servo_yaml = load_yaml(PathJoinSubstitution(
                               [FindPackageShare('dual_robotiq_moveit_config'),
