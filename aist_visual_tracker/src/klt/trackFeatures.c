...
#include "base_.h"
#include "error_.h"
#include "convolve_.h"		/* for computing pyramid */
#include "klt/klt.h"
#include "klt/klt_util.h"	/* _KLT_FloatImage */
#include "pyramid_.h"		/* _KLT_Pyramid */
...
static float
_interpolate(float x, float y, _KLT_FloatImage img)
{
...
#ifndef _DNDEBUG
    if (xt < 0 || yt < 0 || xt + 1 >= img->ncols || yt + 1 >= img->nrows)
    {
    ...
    assert (xt >= 0 && yt >= 0 && xt + 1 < img->ncols && yt + 1 < img->nrows);
...
static void
_compute2by2GradientMatrix(_FloatWindow gradx, _FloatWindow grady,
...
static _FloatWindow
_allocateFloatWindow(int width, int height)
{
    _FloatWindow fw;
...
static float
_sumAbsFloatWindow(_FloatWindow fw, int width, int height)
{
    float sum = 0.0;
    int w;
    ...
    int lighting_insensitive)  /* whether to normalize for gain and bias */
{
  ...
  /* Iteratively update the window position */
	...
	if ( x1 - hw < 0.0f ||  x1 + hw + one_plus_eps >= nc ||
	    *x2 - hw < 0.0f || *x2 + hw + one_plus_eps >= nc ||
	     y1 - hh < 0.0f ||  y1 + hh + one_plus_eps >= nr ||
	    *y2 - hh < 0.0f || *y2 + hh + one_plus_eps >= nr)
	{
	    status = KLT_OOB;
	    break;
	}
...
static KLT_BOOL
_outOfBounds(float x, float y, int ncols, int nrows,
...
static int
_am_gauss_jordan_elimination(float **a, int n, float **b, int m)
{
  /* re-implemented from Numerical Recipes in C */
	...
	{
	    for (l=0;l<n;l++)
		SWAP_ME(a[row][l],a[col][l])
	    for (l=0;l<m;l++)
		SWAP_ME(b[row][l],b[col][l])
	}
	indxr[i] = row;
	indxc[i] = col;
	if (a[col][col] == 0.0)
...
static void
_am_getSubFloatImage(_KLT_FloatImage img,	/* image */
		     float x, float y,		/* center of window */
...
static void
_am_compute6by6GradientMatrix(_FloatWindow gradx,
			      _FloatWindow grady,
			      int width,   /* size of window */
			      int height,
...
static void
_am_compute6by1ErrorVector(_FloatWindow imgdiff,
			   _FloatWindow gradx,
			   _FloatWindow grady,
			   int width,   /* size of window */
			   int height,
...
static void
_am_compute4by4GradientMatrix(_FloatWindow gradx,
			      _FloatWindow grady,
			      int width,   /* size of window */
			      int height,
			      float **T)  /* return values */
...
static void
_am_compute4by1ErrorVector(_FloatWindow imgdiff,
			   _FloatWindow gradx,
			   _FloatWindow grady,
			   int width,   /* size of window */
			   int height,
    ...
    float	    *Ayy)        /* used affine mapping */
{
  ...
  /* Iteratively update the window position */
	...
	if(!affine_map) {
	  ...
	  /* If out of bounds, exit loop */
	    if ( x1 - hw < 0.0f ||  x1 + hw + one_plus_eps >= nc1 ||
		*x2 - hw < 0.0f || *x2 + hw + one_plus_eps >= nc2 ||
		 y1 - hh < 0.0f ||  y1 + hh + one_plus_eps >= nr1 ||
		*y2 - hh < 0.0f || *y2 + hh + one_plus_eps >= nr2)
	    {
		status = KLT_OOB;
		break;
	    }
	  ...
	  /* Compute gradient and difference windows */
		...
		_computeIntensityDifferenceLightingInsensitive(
		    img1, img2, x1, y1, *x2, *y2, width, height, imgdiff);
		_computeGradientSumLightingInsensitive(
		    gradx1, grady1, gradx2, grady2, img1, img2, x1, y1,
		    *x2, *y2, width, height, gradx, grady);
	    }
	    else
	    {
		_computeIntensityDifference(img1, img2, x1, y1, *x2, *y2,
	    ...
	    sprintf(fname, "./debug/kltimg_trans_diff_win%03d.%03d.pgm",
		    glob_index, counter);
	    printf("%s\n", fname);
	  ...
	  /* Use these windows to construct matrices */
	    _compute2by2GradientMatrix(gradx, grady, width, height,
				       &gxx, &gxy, &gyy);
	    _compute2by1ErrorVector(imgdiff, gradx, grady,
				    width, height, step_factor,
				    &ex, &ey);
	  ...
	  /* If out of bounds, exit loop */
...
#ifdef DEBUG_AFFINE_MAPPING
	    counter++;
	    _am_computeAffineMappedImage(img1, x1, y1,
					 1.0, 0.0 , 0.0, 1.0,
					 width, height, imgdiff);
	    aff_diff_win->data = imgdiff;
	    sprintf(fname, "./debug/kltimg_aff_diff_win%03d.%03d_1.pgm",
		    glob_index, counter);
	    printf("%s\n", fname);
	    _KLTWriteAbsFloatImageToPGM(aff_diff_win, fname,256.0);
	    ...
	    _am_computeAffineMappedImage(img2, *x2, *y2,
					 *Axx, *Ayx, *Axy, *Ayy,
					 width, height, imgdiff);
	    aff_diff_win->data = imgdiff;
	    sprintf(fname, "./debug/kltimg_aff_diff_win%03d.%03d_2.pgm",
		    glob_index, counter);
	    printf("%s\n", fname);
	    _KLTWriteAbsFloatImageToPGM(aff_diff_win, fname,256.0);
#endif
	    ...
	    _am_getGradientWinAffine(gradx2, grady2, *x2, *y2,
				     *Axx, *Ayx , *Axy, *Ayy,
				     width, height, gradx, grady);
	    ...
	    {
	      case 1:
		_am_compute4by1ErrorVector(imgdiff,
		...
		*Axy = -(*Ayx);
		...
		break;
	      case 2:
		_am_compute6by1ErrorVector(imgdiff,
					   gradx, grady, width, height, a);
...
void KLTTrackFeatures(KLT_TrackingContext tc, KLT_PixelType *img1,
		      KLT_PixelType *img2, int ncols, int nrows,
		      KLT_FeatureList featurelist)
{
  ...
  /* pyramid, and computing gradient pyramids */
	...
	pyramid1 = _KLTCreatePyramid(ncols, nrows,
				     (int) subsampling, tc->nPyramidLevels);
	_KLTComputePyramid(floatimg1, pyramid1, tc->pyramid_sigma_fact);
	pyramid1_gradx = _KLTCreatePyramid(ncols, nrows,
					   (int) subsampling,
					   tc->nPyramidLevels);
	pyramid1_grady = _KLTCreatePyramid(ncols, nrows,
					   (int) subsampling,
					   tc->nPyramidLevels);
  ...
  /* Do the same thing with second image */
    ...
    pyramid2 = _KLTCreatePyramid(ncols, nrows,
				 (int) subsampling, tc->nPyramidLevels);
    _KLTComputePyramid(floatimg2, pyramid2, tc->pyramid_sigma_fact);
    pyramid2_gradx = _KLTCreatePyramid(ncols, nrows,
				       (int) subsampling, tc->nPyramidLevels);
    pyramid2_grady = _KLTCreatePyramid(ncols, nrows,
				       (int) subsampling, tc->nPyramidLevels);
    for (i = 0 ; i < tc->nPyramidLevels ; i++)
	...
	if (featurelist->feature[indx]->val >= 0)
	{
	  ...
	  /* Beginning with coarsest resolution, do ... */
		...
		val = _trackFeature(xloc, yloc,
				    &xlocout, &ylocout,
				    pyramid1->img[r],
				    pyramid1_gradx->img[r],
				    pyramid1_grady->img[r],
				    pyramid2->img[r],
				    pyramid2_gradx->img[r],
				    pyramid2_grady->img[r],
	  ...
	  /* Record feature */
		...
		{ /*for affine mapping*/
			...
			featurelist->feature[indx]->aff_img_grady
				...
				(tc->affine_window_width+border),
				(tc->affine_window_height+border));
			_am_getSubFloatImage(
			    pyramid1->img[0], xloc, yloc,
			    featurelist->feature[indx]->aff_img);
			_am_getSubFloatImage(
			    pyramid1_gradx->img[0], xloc, yloc,
			    featurelist->feature[indx]->aff_img_gradx);
			_am_getSubFloatImage(
			    pyramid1_grady->img[0], xloc, yloc,
			    featurelist->feature[indx]->aff_img_grady);
